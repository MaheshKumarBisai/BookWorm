...
async function seedGenres() {
  const genresData = [
    { name: 'Fiction', imageUrl: 'https://images.unsplash.com/photo-1544947950-fa07a98d237f?auto=format&fit=crop&w=400&q=80', description: 'Immerse yourself in captivating fictional worlds' },
    { name: 'Non-Fiction', imageUrl: 'https://images.unsplash.com/photo-1512820790803-83ca734da794?auto=format&fit=crop&w=400&q=80', description: 'Discover real stories and factual insights' },
    { name: 'Science Fiction', imageUrl: 'https://images.unsplash.com/photo-1518770660439-4636190af475?auto=format&fit=crop&w=400&q=80', description: 'Explore futuristic and imaginative narratives' },
    { name: 'Mystery', imageUrl: 'https://images.unsplash.com/photo-1515879218367-8466d910aaa4?auto=format&fit=crop&w=400&q=80', description: 'Unravel thrilling mysteries and suspense' },
    { name: 'Romance', imageUrl: 'https://images.unsplash.com/photo-1474552226712-ac0f0961a954?auto=format&fit=crop&w=400&q=80', description: 'Experience heartwarming love stories' },
    { name: 'Biography', imageUrl: 'https://images.unsplash.com/photo-1457369804613-52c61a468e7d?auto=format&fit=crop&w=400&q=80', description: 'Read about remarkable lives and achievements' },
    { name: 'Self-Help', imageUrl: 'https://images.unsplash.com/photo-1532012197267-da84d127e765?auto=format&fit=crop&w=400&q=80', description: 'Transform your life with practical guidance' },
    { name: 'History', imageUrl: 'https://images.unsplash.com/photo-1461360370896-922624d12aa1?auto=format&fit=crop&w=400&q=80', description: 'Journey through time and historical events' },
  ];
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/auth/login', async (req, res) => {
    try {
      ...
      res.json({ message: 'Logged in successfully', user: { ...user, passwordHash: undefined } });
    } catch (error: any) {
  ...
  app.post('/api/auth/forgot-password', async (req, res) => {
    try {
      ...
      console.log(`Password reset link: ${process.env.REPLIT_DEV_DOMAIN || 'http://localhost:5000'}/reset-password?token=${resetToken}`);
  ...
  app.post('/api/profile/avatar', requireAuth, upload.single('avatar'), async (req, res) => {
    try {
      ...
      const user = await storage.updateUser(req.session.userId!, { avatarUrl: dataUrl });
  ...
  app.get('/api/books/:id/download', async (req, res) => {
    ...
    if (!book.downloadUrl) {
      return res.status(404).json({ error: 'Download not available for this book' });
    }
